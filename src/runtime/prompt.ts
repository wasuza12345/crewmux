...
export function harnessPreamble(project: string, role: string, agentId: string, sessionId: string): string {
  return [
    ...
    '- update_board(name, board): the web status board the human watches — write/refresh one when a plan, release check, review, debug session or handoff changes state (layouts: guide("board <kind>"))',
